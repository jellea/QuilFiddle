 ...
 */
replgoog.define('replgoog.asserts.ENABLE_ASSERTS', replgoog.DEBUG);
 ...
 * @param {!replgoog.asserts.AssertionError} e The exception to be handled.
 */
replgoog.asserts.DEFAULT_ERROR_HANDLER = function(e) { throw e; };


/**
 * The handler responsible for throwing or logging assertion errors.
 * @private {function(!replgoog.asserts.AssertionError)}
 */
replgoog.asserts.errorHandler_ = replgoog.asserts.DEFAULT_ERROR_HANDLER;
...
replgoog.asserts.doAssertFailure_ =
  ...
  var e = new replgoog.asserts.AssertionError('' + message, args || []);
  replgoog.asserts.errorHandler_(e);
};
 ...
 * messages.
 * @param {function(replgoog.asserts.AssertionError)} errorHandler
 */
replgoog.asserts.setErrorHandler = function(errorHandler) {
  if (replgoog.asserts.ENABLE_ASSERTS) {
    replgoog.asserts.errorHandler_ = errorHandler;
  }
};
...
replgoog.asserts.fail = function(opt_message, var_args) {
  if (replgoog.asserts.ENABLE_ASSERTS) {
    replgoog.asserts.errorHandler_(new replgoog.asserts.AssertionError(
...
/**
 * Checks if the value is a DOM Element if replgoog.asserts.ENABLE_ASSERTS is true.
 * @param {*} value The value to check.
 * @param {string=} opt_message Error message in case of failure.
 * @param {...*} var_args The items to substitute into the failure message.
 ...
 * @throws {replgoog.asserts.AssertionError} When the value is not a boolean.
 */
replgoog.asserts.assertElement = function(value, opt_message, var_args) {
  if (replgoog.asserts.ENABLE_ASSERTS && (!replgoog.isObject(value) ||
      value.nodeType != replgoog.dom.NodeType.ELEMENT)) {
    replgoog.asserts.doAssertFailure_('Expected Element but got %s: %s.',
        [replgoog.typeOf(value), value], opt_message,
        Array.prototype.slice.call(arguments, 2));
  }
 ...
 */
replgoog.asserts.assertObjectPrototypeIsIntact = function() {
  for (var key in Object.prototype) {
    replgoog.asserts.fail(key + ' should not be enumerable in Object.prototype.');
  }
};
